...
PARAMS = {
    ...
    'epochs': 20,
    'batch_size': 128,
    'input_shape': (500, 2, 1)  # Input shape based on n_points, 2 coordinates (x, y), and 1 channel
}
        ...
        if os.path.isdir(class_dir):
            for filename in os.listdir(class_dir):
                if filename.endswith(".pcd"):
                    ...
                    data.append(points[:, :, np.newaxis])  # Add a new axis for channel dimension
...
# Model Preparation
model = Sequential()
model.add(Conv2D(filters=128, kernel_size=(3, 2), activation="relu", input_shape=PARAMS['input_shape']))
model.add(MaxPooling2D(pool_size=(2, 1)))
model.add(BatchNormalization())
model.add(Conv2D(filters=256, kernel_size=(3, 1), activation="relu"))
model.add(MaxPooling2D(pool_size=(2, 1)))
model.add(BatchNormalization())
model.add(Conv2D(filters=512, kernel_size=(2, 1), activation="relu"))
model.add(MaxPooling2D(pool_size=(2, 1)))
model.add(BatchNormalization())
model.add(Flatten())
